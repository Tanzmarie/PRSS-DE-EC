...
ggplot(meanecon, aes(x = p, y = MCosts, color = Algorithm)) +
  ...
  geom_ribbon(
    data = left_join(lowecon, highecon, by = c("p", "h"), suffix = c("_low", "_high")),
    aes(x = p, ymin = LCosts_low , ymax = UCosts_high, group = h),
  ...
  facet_wrap(~ h, nrow = 2, ncol = 2, scales = "free", 
