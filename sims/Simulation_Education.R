...
# Tests
result_tests <- calculate_tests(n = 250, rho = prevalence_at_target)
print(result_tests)
...
income_data <- rlnorm(500, meanlog = log(27), sdlog = sqrt(3))
...
tau = unlist(result_tests["10% Positive"])
...
result_costs = cbind(result_costs, result_tests["10% Positive"], result_tests["Waiting.Times"])
result_costs = result_costs[,c("Algorithm","10% Positive", "Waiting.Times", "DC","CS","CO","Costs")]
