...
for (mu_value in res) {
  ...
  for (i in seq_along(tau)) {
    ...
    current_costs = calculateEconomicCosts(cv, cm, cp, cl, current_tau, tau0, h, current_omega, n, mu_value, co)
...
result_costs = do.call(rbind, lapply(names(economic_costs_list), function(mu) {
  data.frame(
    Time = rep(prev, each = nrow(economic_costs_list[[mu]][[1]])),
...
ggplot(result_costs, aes(x = Time, y = Costs, color = Algorithm)) +
  ...
  facet_wrap(~ MU, nrow = 4, ncol = 2, scales = "free_y", labeller = label_both) +
